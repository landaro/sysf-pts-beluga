...
There we use translation functions, defined recursively over the respective syntactic languages, to map the terms and types of one system into the respective other.
...
\begin{align*}
  \typingF{}{s}{A} &\iff \typingL{}{\lfloor s \rfloor}{\lfloor A \rfloor} \mAnd \typingL{}{\lfloor A \rfloor}{\Prp}\\
  \typingL{}{a}{b} \mAnd \typingL{}{b}{\Prp} &\iff \typingF{}{\lceil a \rceil}{\lceil b \rceil}
\end{align*}
...
However, working with the untyped syntax felt wrong in sense, as only well-formed types and well-typed terms carry actual meaning.
...
The second formalisation (Section~\ref{sec:abella}) is done in Abella, using higher-order abstract syntax (HOAS).
...
The third proof (Section~\ref{sec:beluga}) is done in Beluga, also following the HOAS design.
...
\item We propose that our equivalence proof serves as a benchmark for reasoning about and relating multiple type systems and languages that involve variable binding.
  ...
  As such it can be seen as a complement to the POPLmark challenge~\cite{poplmark} which tests a frameworks ability to develop the theory of a single object language and type system.
\end{itemize}
